...
def draw_buttons():
    ...
    img = pygame.image.load('search-icon-small.jpg')
    img1 = pygame.image.load('help-icon-small.jpg')
    screen.blit(img1, (help_button_center[0] - 10, help_button_center[1] - 15))
    screen.blit(img, (search_button_center[0] - 10, search_button_center[1] - 15))
...
def open_help_window():
    help_screen = pygame.display.set_mode([800, 600])
...
def open_search_window():
    ...
    while running:
        ...
        if selected_category:
            ...
            elements_text = [f"{elem[0]} ({elem[2]})  Atomic Number : {elem[1]}  Atomic Weight : {elem[3]}" for elem in elements_list]
...
while run:
    screen.fill('black')
    pygame.display.set_caption('Elemental Explorer')
    image = pygame.image.load("search-icon.jpg")
